...
const HorizontalBarPanel = ({
  ...
  return (
    <div className="flex-1 min-w-0">
      <p className="font-bold text-sm text-fm-green mb-3">{title}</p>
      <div className="space-y-2">
        {data.map((d) => (
          <div key={d.year} className="flex items-center gap-2">
            <span className="text-[10px] font-bold text-fm-green w-8 shrink-0 text-right">
              {d.year}
            </span>
            <div className="flex-1 h-5 flex items-center">
              ...
              >
                <span className="text-[8px] font-bold text-white px-1 whitespace-nowrap select-none">
                  {inView && (
